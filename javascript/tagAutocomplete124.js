...
function readFile(filePath) {
  ...
  request.open("GET", filePath + "?ver=124", false);
  request.send(null);
...
function getTextAreaIdentifier(textArea) {
  let txt2img_p = gradioApp().querySelector(
    "#txt2img_prompt > label > textarea"
  );
  let txt2img_n = gradioApp().querySelector(
    "#txt2img_neg_prompt > label > textarea"
  );
  let img2img_p = gradioApp().querySelector(
    "#img2img_prompt > label > textarea"
  );
  let img2img_n = gradioApp().querySelector(
...
function createCheckbox() {
  ...
  label.setAttribute(
    "class",
    '"flex items-center text-gray-700 text-sm rounded-lg cursor-pointer dark:bg-transparent'
  );
...
function isVisible(textArea) {
  let textAreaId = getTextAreaIdentifier(textArea);
  let resultsDiv = gradioApp().querySelector(
    ".autocompleteResults" + textAreaId
  );
...
function insertTextAtCursor(textArea, result, tagword) {
  ...
  if (tagType === "wildcardFile") {
    sanitizedText = text.replace("Wildcards: ", "");
  } else if (tagType === "wildcardTag") {
    sanitizedText = text.replace(/^.*?: /g, "");
  } else if (tagType === "embedding") {
    sanitizedText = `<${text.replace(/^.*?: /g, "")}>`;
  } else {
    sanitizedText = acConfig.replaceUnderscores
      ? text.replaceAll("_", " ")
      : text;
  }

  if (sanitizedText.indexOf("|")) {
    console.log("sanitizedText", sanitizedText);
    sanitizedTextArr = sanitizedText.split("|");
    if (sanitizedTextArr.length > 1) {
      if (tagType === "wildcardTag") {
        sanitizedText = sanitizedTextArr[1];
        if (sanitizedText.endsWith(",")) {
          sanitizedText = sanitizedText.substring(0, sanitizedText.length - 2);
        }
      } else {
        sanitizedText = sanitizedTextArr[0];
      }
    }
  }

  if (tagType === "wildcardFile") {
    sanitizedText = "__" + sanitizedText + "__";
  }
  ...
  if (tagType !== "wildcardFile") {
    optionalComma =
      surrounding.match(new RegExp(escapeRegExp(`${tagword},`))) !== null
        ? ""
        : ", ";
  }
  ...
  // Add back start
  var newPrompt =
    prompt.substring(0, editStart) + insert + prompt.substring(editEnd);
...
function addResultsToList(textArea, results, tagword, resetList) {
  let textAreaId = getTextAreaIdentifier(textArea);
  let resultDiv = gradioApp().querySelector(
    ".autocompleteResults" + textAreaId
  );
  ...
  let nextLength = Math.min(
    results.length,
    resultCount + acConfig.resultStepLength
  );
...
function autocomplete(textArea, prompt, fixedTag = null) {
  ...
  if (
    acConfig.useWildcards &&
    [...tagword.matchAll(/\b__([^,_ ]+)__([^, ]*)\b/g)].length > 0
  ) {
    // Show wildcards from a file with that name
    wcMatch = [...tagword.matchAll(/\b__([^,_ ]+)__([^, ]*)\b/g)];
    let wcFile = wcMatch[0][1];
    let wcWord = wcMatch[0][2];
    results = wildcards[wcFile]
      .filter(x => (wcWord !== null ? x.toLowerCase().includes(wcWord) : x)) // Filter by tagword
      .map(x => [wcFile + ": " + x.trim(), "wildcardTag"]); // Mark as wildcard
  } else if (
    acConfig.useWildcards &&
    ((tagword.startsWith("__") && !tagword.endsWith("__")) || tagword === "__")
  ) {
    // Show available wildcard files
    let tempResults = [];
    if (tagword !== "__") {
      tempResults = wildcardFiles.filter(x =>
        x.toLowerCase().includes(tagword.replace("__", ""))
      ); // Filter by tagword
    } else {
    ...
    let tempResults = [];
    if (tagword !== "<") {
      tempResults = embeddings.filter(x =>
        x.toLowerCase().includes(tagword.replace("<", ""))
      ); // Filter by tagword
    } else {
      tempResults = embeddings;
    }
    // Since some tags are kaomoji, we have to still get the normal results first.
    genericResults = allTags
      .filter(x => x[0].toLowerCase().includes(tagword))
      .slice(0, acConfig.maxResults);
    results = genericResults.concat(
      tempResults.map(x => ["Embeddings: " + x.trim(), "embedding"])
    ); // Mark as embedding
  } else {
    if (acConfig.translation.searchByTranslation) {
      results = allTags.filter(
        x => x[2] && x[2].toLowerCase().includes(tagword)
      ); // check have translation
      ...
      if (!acConfig.translation.onlyShowTranslation) {
        results = results.concat(
          allTags.filter(
            x => x[0].toLowerCase().includes(tagword) && !results.includes(x)
          )
        );
      }
    } else {
...
function navigateInList(textArea, event) {
  ...
  if (acConfig.useLeftRightArrowKeys) validKeys.push("ArrowLeft", "ArrowRight");
...
onUiUpdate(function() {
        ...
        if (acConfig.extra.onlyTranslationExtraFile) {
          ...
          extras.forEach(e => {
            ...
            if ((tag = allTags.find(t => t[0] === e[0] && t[1] == e[1]))) {
              if (
                e[2] // If the extra tag has a translation, add it to the tag
              )
                tag[2] = e[2];
            } else {
  ...
  if (wildcardFiles.length === 0 && acConfig.useWildcards) {
    try {
      wildcardFiles = readFile("file/tags/temp/wc.txt")
        .split("\n")
      ...
      wildcardFiles.forEach(fName => {
        try {
          wildcards[fName] = readFile(`file/scripts/wildcards/${fName}.txt`)
            .split("\n")
            .filter(x => x.trim().length > 0); // Remove empty lines
        } catch (e) {
  ...
  if (embeddings.length === 0 && acConfig.useEmbeddings) {
    try {
      embeddings = readFile("file/tags/temp/emb.txt")
        .split("\n")
        .filter(x => x.trim().length > 0) // Remove empty lines
  ...
  // Find all textareas
  let txt2imgTextArea = gradioApp().querySelector(
    "#txt2img_prompt > label > textarea"
  );
  let img2imgTextArea = gradioApp().querySelector(
    "#img2img_prompt > label > textarea"
  );
  let txt2imgTextArea_n = gradioApp().querySelector(
    "#txt2img_neg_prompt > label > textarea"
  );
  let img2imgTextArea_n = gradioApp().querySelector(
    "#img2img_neg_prompt > label > textarea"
  );
  let textAreas = [
    txt2imgTextArea,
  ...
  if (gradioApp().querySelector(".autocompleteResults.p")) {
    if (
      gradioApp().querySelector(".autocompleteResults.n") ||
  ...
  textAreas.forEach(area => {
    ...
    if (
      (!acConfig.activeIn.img2img && textAreaId.includes("img2img")) ||
    ...
    if (!area.classList.contains("autocomplete")) {
      ...
      area.addEventListener(
        "input",
        debounce(() => autocomplete(area, area.value), 100)
      );
  ...
  let css = gradioApp().querySelector(".dark")
    ? autocompleteCSS_dark
    : autocompleteCSS_light;
